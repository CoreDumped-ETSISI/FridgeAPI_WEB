...
function card(element, funcName) {
  return `<div class="col s6 m4 l3 xl2">
          ...
          <div id="${element._id}" class="card-image" onclick="${funcName}('${
    element._id
  }')">
          <img src="${element.image}"/>
          <span class="card-title" id="product-name">${element.name}</span>
          <span class="new badge" id="product-stock" data-badge-caption=" UDS ">${
            element.stock
          }</span>
          <a class="btn red" id="product-price">${
            element.price
          }€</a></div></div></div>`;
}
...
function addOfferToCart(id) {
  ...
  if (offerStock > 0) {
    ...
    for (let i = 0; i < offer.products.length; i++) {
      ...
      if (stock >= offer.products[i].quantity) {
        is_stock = true;
      } else {
        is_stock = false;
      }
  ...
  $("#cartList").append(
    `<li class="cartElement valign-wrapper">
      <img class="responsive-img" src="${
        element.image
      }" style="width: 15%; margin-left: 5%; object-fit: contain;">
      <span style="width: 65%; padding-left: 1em;">${element.name}</span>
      <i class="material-icons" onclick="${eraseFuncName}('${
      element._id
    }', '${index}', '${element.name}')" style="cursor: pointer;">clear</i>
      </li>`
  );
}
...
function updateStock(productId, quantity) {
  ...
  let stock = parseInt(productStock.text());
  if (stock >= 0) productStock.text(stock + quantity);
}
    ...
    if (products.length > 0 || offers.length > 0) {
      if (
        ...
        request("POST", "/purchase", data, res => {
          ...
          getOfferList();
          M.toast({
            html: "Su compra se ha realizado correctamente",
            classes: "green"
          });
          buying = false;
        });
        ...
        M.toast({
          html: "No tienes saldo suficiente para efectuar la compra",
          classes: "orange"
        });
      }
    } else {
...
function listIsEmpty() {
  $("#cartList").append(
    '<span id="empty" style="width: 100%; color:#aaa;">No hay productos en el carrito :(</span>'
  );
}
